...
class JSON_Packet:
    ...
    def send(self, obj):
        ...
        j_len = len(j_str)
        j_lrc = lrc(j_str)			#longitudinal redundancy check
        hdr = bytearray((SOH, j_len & 0xff, j_len >> 8, STX)) #header & 0xff masks the lower eight bits(FF is 255) >>8 means shift to the right by 8 bits 
        ftr = bytearray((ETX, j_lrc, EOT))	#footer
